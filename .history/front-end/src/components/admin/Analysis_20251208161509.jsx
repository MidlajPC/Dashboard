  ...
  const handleFilter = () => {
    ...
    if (filterRole && filterRole !== "All") {
      filtered = filtered.filter(
        (log) =>
          log.data[0].operator?.role.toLocaleLowerCase() === filterRole.toLocaleLowerCase()
      );
    }
    ...
    if (startTime && endTime) {
      filtered = filtered.filter((log) => {
            const start = dayjs(logDate.format("YYYY-MM-DD") + " " + startTime);
    const end = dayjs(logDate.format("YYYY-MM-DD") + " " + endTime);
        const logDate = dayjs(log.createdAt).local();
        const itemMinutes =
          logDate.hour() * 60 + logDate.minute();

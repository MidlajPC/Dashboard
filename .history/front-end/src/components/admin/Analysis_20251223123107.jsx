...
const Analysis = () => {
  ...
  useEffect(() => {
    ...
    const operatorName = (log.operators?.[0]?.user?.name ?? "").toLowerCase();
    const operatorEmail = (log.operators?.[0]?.user?.email ?? "").toLowerCase();
    const dataOpName = (log.data?.[0]?.operator?.name ?? "").toLowerCase();
    const dataOpEmail = (log.data?.[0]?.operator?.email ?? "").toLowerCase();
        return (
          (log.UniqueCode || log.name.toLowerCase()).includes(
            search.toLocaleLowerCase()
          ) ||
          (log.operators[0]?.user.name.toLowerCase().includes(
            search.toLocaleLowerCase()) ||
          (log.operators[0]?.user.email.toLowerCase().includes(
            search.toLocaleLowerCase()) ||
          (log.data[0].operator?.name.toLowerCase().includes(
            search.toLocaleLowerCase()) ||
          (log.data[0].operator?.email.toLowerCase().includes(
            search.toLocaleLowerCase())
        );
      });
    }

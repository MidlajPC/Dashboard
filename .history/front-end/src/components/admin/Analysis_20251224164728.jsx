...
const Analysis = () => {
  ...
  useEffect(() => {
    ...
    if (search.trim().length > 0) {
      ...
      filtered = filtered.filter((log) => {
        const botCode = (log.UniqueCode ?? "").toLowerCase();
        const botName = (log.name ?? "").toLowerCase();
        const operatorName = (
          log.operators?.[0]?.user?.name ?? ""
        ).toLowerCase();
        const operatorEmail = (
          log.operators?.[0]?.user?.email ?? ""
        ).toLowerCase();
        const dataOpName = (log.data?.[0]?.operator?.name ?? "").toLowerCase();
        const dataOpEmail = (
          log.data?.[0]?.operator?.email ?? ""
        ).toLowerCase();
        return (
          botCode.includes(searchText) ||
          botName.includes(searchText) ||
          operatorName.includes(searchText) ||
          operatorEmail.includes(searchText) ||
          dataOpName.includes(searchText) ||
          dataOpEmail.includes(searchText)
        );
      });
    }
  ...
  const downloadExcel = () => {
    const worksheet = XLSX.utils.json_to_sheet(flattenedAnalysis);
                      ...
                      <ul className="dropdown-ul">
                        ...
                        >
                          <div onClick={roleDropdown} className="dropdown">
                            <div className="dropdown-div2 !pb-4">
                          ...
                          {isRoleDropdownOpen && (
                            <div
                              ...
                              // }}
                            >
                              <div className="inner-options-div !bg-white !text-black !right-0  dropdown-glass">
                                <ul className="dropdown-ul">
                        ...
                        <div className="flex gap-2 flex-wrap">
                          ...
                          <div className="flex gap-2 mb-3">
                            <TextField
                              ...
                              className="bg-[#1b1f30] rounded-md text-black placeholder-black"
                            />
                            <TextField
                              label="End Time"
                              type="time"

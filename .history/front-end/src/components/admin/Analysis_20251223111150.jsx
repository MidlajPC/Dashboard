...
const Analysis = () => {
  ...
  const [filteredAnalys, setfilteredLogs] = useState([]);
  // useEffect(() => {
  ...
  const downloadPdf = () => {
    ...
    const tabledata = filteredAnalys.flatMap((log, idx) =>
      (log.data ?? [[]])
        .slice()
  ...
  // const flattenedLogs = filteredAnalys
  //   .flatMap((log, idx) =>
  //     log.actions.map((data, didx) => ({
  //       ...action,
  //       logId: log._id,
  //       user: log.user
  //     }))
    ...
    <div className="m-2 flex flex-col">
      ...
      <div className="tbldiv">
        <table className="tbl">
          ...
          <tbody>
            {filteredAnalys?.flatMap((log, idx) => {

...
const botHistorySchema = new mongoose.Schema({}, { strict: false });
const botHistoryModel = mongoose.model("Bot_History", botHistorySchema, "Bot_History");
  ...
  const sendBotData = async (target) => {
    try {
      const bots = await botModel
        ...
        .populate({ path: "data.operator", select: "name email" })
        .populate({ path: "operators.user", select: "-password" });
      const history = await botHistoryModel.find().populate({ path: "data.operator", select: "name email" })
        .populate({ path: "operators.user", select: "-password" });
      target.emit("botData", { botData: bots, botHistory: history });
    } catch (error) {
